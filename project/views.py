...
from .models import File, Post , URL, Video
...
def detail_view(request, id ):
    ...
    photo = File.objects.filter(extendeds=post)
    url = URL.objects.filter(urls=post)
    titulo = Post.objects.filter(title=post)
    video = Video.objects.filter(vid=post)
